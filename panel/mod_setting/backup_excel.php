...
$nama_mapel = mysqli_fetch_array(mysqli_query($koneksi, "SELECT kode FROM mapel WHERE id_mapel='$mapel_id'"));
...
$soal_mapel = mysqli_query($koneksi, "SELECT * FROM soal WHERE id_mapel='$mapel_id'");

$file_excel = $mapel_id . "-" . $nama_mapel['kode'] . ".xlsx";
$file_excel_old = $mapel_id . "-" . $nama_mapel['kode'] . ".xls";
...
// backup zip
$mapel = mysqli_query($koneksi, "SELECT `file`, file1, fileA, fileB, fileC, fileD, fileE FROM soal WHERE id_mapel='$mapel_id'");
...
if (isset($array)) :
    $backup_file = 'backup_soal/' . $mapel_id . "-" . $nama_mapel['kode'] . '.zip';
...
?>
    <div class="alert alert-success alert-dismissable">
        ...
        Master soal <?= $nama_mapel['kode']; ?> telah diproses.<br />
        Silahkan mendownload dengan mengklik
        <a href="mod_setting/<?= $backup_file ?>"><button class="btn btn-info"> download file</button></a>
    </div>
<?php else : ?>
    <div class="alert alert-danger alert-dismissable">
        <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
        Master soal <?= $nama_mapel['kode']; ?> gagal diproses.<br />
    </div>
<?php endif; ?>